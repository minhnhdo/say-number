...
fn say_0(c: &mut Criterion) {
    c.bench_function("say 0 in English", |b| b.iter(|| say(black_box(0))));
}

fn say_42(c: &mut Criterion) {
    c.bench_function("say 42 in English", |b| b.iter(|| say(black_box(42))));
}

fn say_514(c: &mut Criterion) {
    c.bench_function("say 514 in English", |b| b.iter(|| say(black_box(514))));
}

fn say_max_u64(c: &mut Criterion) {
    c.bench_function("say max u64 in English", |b| {
